...
## 프로젝트 개요
이 프로젝트는 AWS 기반 3-Tier 아키텍처를 직접 설계하고 구축한 프로젝트입니다.

AWS Console을 통해 네트워크, 애플리케이션, 데이터베이스 계층을 구성하고,
보안 그룹, 로드 밸런싱, 오토 스케일링, 모니터링, HTTPS 도메인 연결까지 단계적으로 확장했습니다.

초기 구축 과정을 기준선으로 삼아 운영 고도화와 Terraform 코드화까지 이어지는 흐름을 정리합니다.
...
## 이 프로젝트를 진행한 이유
- 단순 EC2 1대 배포가 아니라 3-Tier 구조를 직접 설계하고 구현
